...
const Chauffeur = ({ navigation, route }: any) => {
  ...
  useEffect(() => {
    const fetchCommandeData = async () => {
      try {
        const commandeDoc = await getDoc(
          doc(firestore, "commandes", commandeId)
        );
        if (commandeDoc.exists()) {
          ...
          if (commandeData.chauffeur) {
            ...
            Alert.alert(
              "Erreur",
              "Les informations du chauffeur ne sont pas disponibles."
            );
          }
        } else {
        ...
        Alert.alert(
          "Erreur",
          "Erreur lors de la récupération des données de la commande: " +
            error.message
        );
      }
    };
  ...
  const deleteCommande = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Erreur",
        `Une erreur est survenue lors de l\'annulation de la commande.,${error.message}`
      );
    } finally {
      setLoading(false);
    }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.overlay}>
        ...
        <Text style={{ marginTop: "2%" }}>
          ARRIVE DANS<Text style={{ color: "#088A4B" }}>~5MIN</Text>
        </Text>
        ...
        <View style={styles.line}></View>
        <TouchableOpacity
          onPress={() => makePhoneCall(chauffeurInfo.phone)}
          style={styles.zoneAppel}
        >
